...
Before you begin it is recommended that you consider upgrading your ProBook. All of its off-the-shelf configurations are hopelessly outdated.
...
### HD+ and Full HD Screen (for 15" Models)
...
The details of this upgrade are beyond the scope of this guide but you can find all necessary information on relevant hackintosh forums. If you do manage to install full HD screen, don’t forget to adjust iGPU device properties to enable dual-link operation or you will experience broken image. Actually, if you own a 4730s model ProBook you will need to do this adjustment regardless because all 17" ProBooks came with HD+ 1600x900 screens installed.

Add the following to `config.plist` under `DeviceProperties/Add/PciRoot(0x0)/Pci(0x2,0x0)` for both regular and USB version before beginning setup process (you will work with this file in the following sections):
...
After deciding on your upgrades it’s time to [start the installation process](2.\ Installation.md).
